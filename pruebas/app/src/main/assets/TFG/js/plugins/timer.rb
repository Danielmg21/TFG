...
#                                               remmonth,remyear
#           GameTime.com.example.limpieza.clock?(true/false)        #hides/shows the com.example.limpieza.clock
...
#          GameTime.com.example.limpieza.clock?(true)
#
#----------#
...
#Clock is shown
USECLOCK = true
#Set to true to have the com.example.limpieza.clock show up in the menu!
USECLOCK_MENU = false
#Set the format for the com.example.limpieza.clock both in and out of menu
#1. hh:mm am/pm
#2. Sun dd hh:mm am/pm
#3. Custom com.example.limpieza.clock, see below
CLOCK_FORMAT = 3
MENU_CLOCK_FORMAT = 3
#Set to true for a Twenty four hour com.example.limpieza.clock
TF_HOUR_CLOCK = false
#Clock window background opacity
CLOCK_BACK = 75
#Whether to use a special image for the back of the com.example.limpieza.clock or not (Picture folder)
...
#Button to be used to toggle the com.example.limpieza.clock
CLOCK_TOGGLE = :SHIFT
#Toggle Options - :off, :hour12, :hour24
CLOCK_TOGGLE_OPTIONS = [:off,:hour12,:hour24]
#X and Y position of com.example.limpieza.clock
CLOCK_X = Graphics.width - 175
CLOCK_Y = Graphics.height - 48 - 24 - 12
#Finetune the width of the com.example.limpieza.clock window here:
CLOCK_WIDTH = 175
#Whether or not those little dots on the com.example.limpieza.clock blink
USE_BLINK = true
#The speed at which they blink
BLINK_SPEED = 120
#Here is where you would insert the array of commands for the custom com.example.limpieza.clock:
...
module GameTime
  ...
  def self.com.example.limpieza.clock?(set)
...
class Scene_Menu
  ...
  def start
    gt_start
    @com.example.limpieza.clock = GameTime::Window_GameClock.new if USECLOCK_MENU
    return if @com.example.limpieza.clock.nil?
    @com.example.limpieza.clock.x = 0
    @com.example.limpieza.clock.y = @gold_window.y - @com.example.limpieza.clock.height
    @com.example.limpieza.clock.width = @gold_window.width
    @com.example.limpieza.clock.create_contents
  end
  def update
    gt_update
    @com.example.limpieza.clock.update unless @com.example.limpieza.clock.nil?
    @com.example.limpieza.clock.contents.clear if SceneManager.scene != self and !@com.example.limpieza.clock.nil?
  end
end
